...
for (let i = 1;  i >= 0; i--){
    ...
    selectElem[0].firstElementChild.insertAdjacentHTML('afterend' ,
    option);
}
...
for (let i = 1;  i >= 0; i--){
    let option  = `<option value=${units[i]}>${units[i]}</option>`;
    selectElem[1].firstElementChild.insertAdjacentHTML('afterend' ,
    option);
}
...
function convertUnits (){
    if (
        ...
        (convertFrom.value === 'meter') && (convertTo.
            value === 'meter') ||
        (convertFrom.value === 'kilometer') && (convertTo.
            value === 'kilometer')
    ){
        ...
        value === 'centimeter')){
            result.value = length.value * 100;
            formula.textContent = hints[3].m_c;
    }
    else if ((convertFrom.value === 'meter') && (convertTo.
        value === 'kilometer')){
            result.value = length.value / 1000;
            formula.textContent = hints[4].m_km;
    }
    else if ((convertFrom.value === 'kilometer') && (convertTo.
        value === 'centimeter')){
            result.value = length.value * 100000;
            formula.textContent = hints[5].km_cm;
    }
    else if ((convertFrom.value === 'kilometer') && (convertTo.
        value === 'meter')){
            result.value = length.value * 1000;
            formula.textContent = hints[6].km_m;
    }
}
